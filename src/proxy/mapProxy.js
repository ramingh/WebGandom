...
export class GandomMap {
    ...
    Draw_buffer(l1, l2, textRadius, map) {
        const Url_domain = 'https://gis.gandomcs.com/arcgis/rest/services/';
        let base_point = [];
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                for (let i = 0; i < data.results.length; i++) {
                    ...
                    name0 = data.results[i].attributes.Name;
                    category0 = data.results[i].attributes.Category;
                    let icooo = category0;
                    if (name0 == '0') {
                        name0 = '';
                    };
                    let txt = category0 + '  <br /> ' + name0;
                    // var ll = getDistance(base_point, Point_mark);

                    if (category0 != '  سوپرمارکت') {
                        let popup2 = this.Route_find(Point_mark, base_point, map);
                        popup2.push(category0, name0);
                        popup1.push(popup2);

                        this.createPolyLine(Point_mark, popup2, map);
                    };
                };
                // Draw_popup(base_point, popup1, map);
            }
        });
    }
  async  Route_find(loc1, loc2, map) {
        ...
        let url = 'https://map.ir/routing/v2/route?alternatives=false&steps=true&geometries=geojson&overview=full&profile=' + selrot + '&coordinates=' + xxyy;
        ...
        //         dist = dist.toFixed(0);
        //         this.timeall = [{ timm, dist }];

        //         // Use arrow function to preserve this context
        //         Geom.forEach((item) => {
        //             let pnt = new L.LatLng(item[1], item[0]);
        //             this.araypoint.push(pnt);
        //         });
        //         this.Draw_line(this.araypoint, timee, map);
        //     }
        // });
        return this.timeall;
    }
